...
Each node has a private and public key pair, and a node address. ClishaChain uses the private and public key
pair to sign and verify transactions, and the node address as an identifier for the node.

## Node private key

When starting ClishaChain, if the
[`--node-private-key-file`](../reference/cli/options.md#node-private-key-file) option is not
specified and a `key` file does not exist in the data directory for the node, ClishaChain generates a node
private key and writes it to the `key` file.

If a `key` file does exist in the data directory when starting ClishaChain, the node starts using the
private key in the `key` file.


## Node public key

The node public key displays in the log after starting ClishaChain. Also referred to as the node ID, the
...
ClishaChain generates the node address by creating a hash of the node public key and using the last 20
bytes of the hash as the node address. It is also displayed in the logs after starting ClishaChain.
...
does not exist, ClishaChain generates a node private key and writes it to the `key` file.
...
The enode URL displays when starting a ClishaChain node. Use the
